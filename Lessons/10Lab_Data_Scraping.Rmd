...
#Get the data, which starts on line 30, and only get last 24 days
gage_data2 <- read.table(theURL2,skip = 30,header=TRUE,sep='\t',stringsAsFactors = T) 
...
nc_pop <- 
  get_acs(geography = "county", #get_acs is part of the tidycenus package
          variables = "B01003_001", #code for overall population; you can look up other codes
          state = "NC", #filter by geography
          geometry = TRUE)
...
the_states <- the_website %>% html_nodes('tr :nth-child(1)') %>% html_text()
the_price <- the_website %>% html_nodes('tr :nth-child(2)') %>% html_text()
the_capacity <- the_website %>% html_nodes('tr :nth-child(3)') %>% html_text()
net_generation <- the_website %>% html_nodes('tr :nth-child(4)') %>% html_text()
total_retail <- the_website %>% html_nodes('tr :nth-child(5)') %>% html_text()
  
#3 Construct a dataframe from the values
df_states_energy <- data.frame(
  'State' = the_states,
  'Price' = as.numeric(the_price), #make the numbers into numerals
  'Capacity' = as.numeric(gsub(',','', the_capacity)), #gsub used to tell as.numeric what things are also included 
  'Net Generation' = as.numeric(gsub(",","",total_retail))
  )
```
...
df_years <- lapply(seq(2017,2022),scrape.it) %>% #X tells what to iterate the function over, then put in the function
            bind_rows() 
