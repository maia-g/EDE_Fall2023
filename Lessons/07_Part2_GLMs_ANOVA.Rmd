...
Null: there is no difference in the mean
...
summary(Litter.Totals$plotID) #shows number of samples per site
...
#shows pval is really small whcih means we reject the null
...
#clearly shows data does not follow a normal distribution; doesn't mean we can't run the test
...
#Main difference between AOV and LM is the summary output. lm has one row for each value of categorical variable (LEVELS)
...
#shows some means are simialr, others are very different

# Extract groupings for pairwise relationships
Litter.Totals.groups <- HSD.test(Litter.Totals.anova, "plotID", group = TRUE) #provide with anove object, the category, and telling you want it to group
Litter.Totals.groups
#this will find groups that have similar means
...
> You would want to make the plot in the same order as the groupings were provided rather than from small site id to large.
...
```{r}
# Format as aov
Litter.anova.2way <- aov(data = Litter, dryMass ~ functionalGroup + nlcdClass) #one row for each categorical variable
summary(Litter.anova.2way)
#rejecting the null but doesnt tell us what groups are different
...
# Run a post-hoc test for pairwise differences to see what groups are different!
...
#reject the null, there is significant interaction between the vars
...
#shows all possible combinations of vars and groups; 8 levels for function group and 3 for class therefore 24 rows

#Trying to find treatment groups
...
#compares means for each level of the anova

#Then find treatment groups of interactions
