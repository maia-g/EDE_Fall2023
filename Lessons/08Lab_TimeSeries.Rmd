...
##In the assignment, we will do this a better way for timeseries
sum(is.na(Temp_BR_processed$Temperature_C))
sum(is.na(Temp_BR_processed$Date))

sum(is.na(Rainfall_BR_processed$Date))
sum(is.na(Rainfall_BR_processed$Rainfall))

#Neither data set has any NA values
...
temp_plot <-
  ggplot(Temp_BR_processed,
         aes(x = Date, y = Temperature_C)) +
  geom_line()+
  geom_smooth(method = lm)

temp_plot 
#Temp shows small upward trend with high variability, appears seasonal

rain_plot <-
  ggplot(Rainfall_BR_processed,
         aes(x = Date, y = Rainfall)) +
  geom_line() +
  geom_smooth(method = lm)

rain_plot
#Rain shows no trend, but super high variability, appears seasonal as well
```
...
temp_data_ts <- ts(Temp_BR_processed$Temperature_C,
                   start=c(Temp_f_year,Temp_f_month),
                   frequency=12) ##This says how many data points per year we have (we have monthly data so we used 12)

Rain_f_month <- month(first(Rainfall_BR_processed$Date))
Rain_f_year <- year(first(Rainfall_BR_processed$Date))
rain_data_ts <- ts(Rainfall_BR_processed$Rainfall,
                   start=c(Rain_f_year,Rain_f_month),
                   frequency=12)
```
...
temp_data_decomp <- stl(temp_data_ts,s.window = "periodic") ##Periodic takes the number "12" we used during timeseries
plot(temp_data_decomp)
##Shows 
rain_data_decomp <- stl(rain_data_ts,s.window = "periodic")
plot(rain_data_decomp)
...
temp_decomp_Components <- as.data.frame(temp_data_decomp$time.series[,1:3])
temp_decomp_Components <- mutate(temp_decomp_Components,
        Observed = Temp_BR_processed$Temperature_C,     ##Adding an observed col to compare with the trend col
        Date = Temp_BR_processed$Date)
## the [1:3] comes from the TS object which you can click on the arrow and see the data frame dimensions which has [1:3] in the col for seasonal, trend, and remainder

rain_decomp_Components <- as.data.frame(rain_data_decomp$time.series[,1:3])
rain_decomp_Components <- mutate(rain_decomp_Components,
...
ggplot(temp_decomp_Components) +
  geom_line(aes(y = Observed, x = Date), color = "lightgray") +
  geom_line(aes(y = trend, x = Date), color = "#c13d75ff") +
  labs(y = "Temperature C")

ggplot(rain_decomp_Components) +
  geom_line(aes(y = Observed, x = Date), color = "lightgray") +
