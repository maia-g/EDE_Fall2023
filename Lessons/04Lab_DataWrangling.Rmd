...
#diff classes are dealt with differently

# dplyr filtering
NTL.phys.data.surface <- filter(NTL.phys.data, depth == 0) # '==' is looking for exact value
#reduces the number of rows bc we are FILTERING

# Choose multiple conditions to filter out two lake names we want
summary(NTL.phys.data$lakename) #shows all the lake names; can also use unique OR levels to see this too; need to do this to get the EXACT names (case sensitive)
NTL.phys.data.PeterPaul <- 
  filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) #the %in% operator is a faster way to ask if something is within the vector that you want

# Choose a range of conditions of a numeric or integer variable
summary(NTL.phys.data$daynum)
NTL.phys.data.JunethruOctober <- filter(NTL.phys.data, daynum %in% c(152:304)) #if the value is within the c() then we keep it
#you can find what day june starts by looking in your data set
```
...
# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999. THIS IS INCLUSIVE.
NTL.phys.data.TuesLake <- filter(NTL.phys.data, lakename == "Tuesday Lake") 
NTL.phys.data.TuesLake.90.99 <-  filter(NTL.phys.data.TuesLake, year4 %in% c(1990:1999))
...
Pipe is another method to wrangle datasets that looks cleaner and is easier to read.  We designate a pipe with `%>%`. A good way to think about the function of a pipe is with the word "then." Shows the FLOW of your calculations. 
...
#to debug, just run one command at a time until you figure out where it went wrong

#Exercise 3: Using a pipe filter NTL.phys.data for Tuesday Lake from 1990 
# through 1999 only for July.
#when you are piping create a month column or before you filter you can convert into a month
NTL.phys.data.TuesLake.July.90.99 <-
  NTL.phys.data.TuesLake.90.99 %>%
  filter(daynum %in% c(181:213))
