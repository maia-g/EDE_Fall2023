...
One variable of left (y) one variable on right (x).

For the NTL-LTER dataset, can we predict irradiance (light level) from depth?

```{r}
#function lm is linear regression, needs to provide it with dependent variable (y) and independent var (x)
...
#there is s strong negative correlation between irradiance and depth
```
Question: How would you report the results of this test (overall findings and report of statistical output)?

>  There is a strong negative correlation between irradiance and depth. Also see paragraph below.
...
2. **Normal Q-Q.** This is comparing a distribution with a normal distribution. The points should fall close to the 1:1 line. We often see departures from 1:1 at the high and low ends of the dataset, which could be outliers. 
...
par(mfrow = c(2,2), mar=c(4,4,4,4)) #dividing plot screen into a grid
plot(irradiance.regression) #call plot and provide r wiht linear model object
par(mfrow = c(1,1))
```
...
irradiancebydepth <- 
  ...
  ylim(0, 2000) + #changing plot window to not show the outlier
  geom_point() 
print(irradiancebydepth) 

#shows that we do NOT have a linear relationship, so want to trasnform it (next chunk)
```
...
PeterPaul.chem.nutrients <- filter(PeterPaul.chem.nutrients, 
                                   irradianceWater != 0 & irradianceWater < 5000) #getting rid of the outliers
irradiance.regression2 <- lm(data = PeterPaul.chem.nutrients, log(irradianceWater) ~ depth) #then running regression again using the LOG of irradiance!!
summary(irradiance.regression2)
#note that this makes the r^2 to 0.7255 instead of 0.30, this makes our regression line better fit to the data

#Now plotting the 4 plots in grid again, they look better than before
...
irradiancebydepth2 <- 
  ...
  scale_y_log10() + #y axis will be the log transformed irradiance, makes a straight line; this also has a gray confidence interval line which you can mess with (see below)
  geom_point() 
...
``` {r, fig.height = 3, fig.width = 4}
TPregression <- lm(data = subset(PeterPaul.chem.nutrients, lakename == "Paul Lake"), #choosing a subset of the data for only Paul Lake
                   tp_ug ~ depth + dissolvedOxygen) #column is tp_ug
summary(TPregression)
#note that p-value is less than 0.05, which is good; the r^2 is only 0.29 which isnt great (only 30% of the variability can be explained)
...
#Plot one: looking for symmetry (this does not have it)
#Plot two: QQ not too bad but does have some variation at the ends
#Plot three: looking for symmetry (same probs as plot 1)
#Plot four: shows some obs are outliers

```

## Correlation Plots
Trying to make regression better!!
We want to figure out what is impacting the tp_ug that are not just depth and oxygen.
...
PeterPaulCorr <- cor(PeterPaulnutrients) #creates a matrix filled wiht numbers
corrplot(PeterPaulCorr, method = "ellipse") #correlation plot matrix
#oval shapes represent correlation (skinny means higher correlation, dark blue means strong positive, dark red means strong negative)
corrplot.mixed(PeterPaulCorr, upper = "ellipse") #just printing upper diagonal with upper being the ovals and lower will be the values

#independent var is tp_ug, we are trying to find what is impacting it, we want a high r^2 but not with TOOOO many variables
```

## AIC to select variables

However, it is possible to over-parameterize a linear model. Adding additional explanatory variables takes away degrees of freedom, and if explanatory variables co-vary the interpretation can become overly complicated. Remember, an ideal statistical model balances simplicity and explanatory power! 

To help with this tradeoff, we can use the **Akaike's Information Criterion (AIC)** to compute a stepwise regression that either adds explanatory variables from the bottom up or removes explanatory variables from a full set of suggested options. The smaller the AIC value, the better. 
...
#new regression with all the vars
TPAIC <- lm(data = Paul.naomit, tp_ug ~ depth + dissolvedOxygen + 
              temperature_C + tn_ug + po4) #calling ALL nutrients as explanatory vars (5 cols of dataset)

#Choose a model by AIC in a Stepwise Algorithm
step(TPAIC)
# Results are showing results when each var is removed one at a time; looking at AIC value and trying to get smaller. If a var is removed and AIC gets worse, you do not want to remove it.

TPmodel <- lm(data = Paul.naomit, tp_ug ~ dissolvedOxygen + temperature_C + tn_ug)
summary(TPmodel)
#The result is actually worse in this case, but usually it makes it better.
