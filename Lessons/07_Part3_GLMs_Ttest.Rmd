...
> Answer: 
H0: mu >= 50 #the = will always be on the null, not the alt
Ha: mu < 50
...
O3.onesample <- t.test(EPAair$Ozone, mu = 50, alternative = "less") #could have also said 'greater' if that was the alt
O3.onesample
#reject the null bc p val is really small
...
print(Ozone.plot)
#plot shows that most values fall to below the green line, which means our avg will also be to the left
...
ggplot(EPAair, aes(x = Ozone, color = as.factor(Year))) +
  geom_freqpoly() #generating freq plot for each year we have
...
O3.twosample
#pval is less than .05 so we reject the null, the ttest tells us the the difference between the mean is not 0
