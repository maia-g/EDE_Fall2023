...
2. Explore the components of times series: trend(up or down), seasonal(up and down associated with calendar events, like summer vs winter or weekday vs weekend), random (plot shows no correltation or trend), cycle (similar to seasonal, but usually has larger periods of duration and not confined by specific calendar events)
...
##The data has different time-steps, so we need to learn how to deal with it.
```
