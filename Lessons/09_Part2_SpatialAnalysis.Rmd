...
epa_pm25_sites_sf <- read_csv(here('Data/Raw/EPAair_PM25_NC2018_raw.csv')) %>% 
  ...
  st_as_sf(coords = c('SITE_LONGITUDE','SITE_LATITUDE'), crs=4269) #convert to spatial features
...
huc8_sf_Neuse <- st_read(here('Data/Spatial/NCHUC8.shp')) %>%
  filter(SUBBASIN == 'Upper Neuse')


#Create a map
mapView(list(huc8_sf, huc8_sf_Neuse), col.regions = list("orange","purple"))
...
unique(huc2_sf$DWQ_Basin)

#Summarize on DWQ Basin value
summary(huc2_sf$DWQ_Basin) ### Is this the goal????

huc2_sf <- huc8_sf %>%
  group_by(DWQ_Basin) %>%
  summarize(Sum_Acres = sum(ACRES), Sum_sq_miles = sum(SQ_MILES))

#Map the data
mapView(huc2_sf, zcol = 'Sum_Acres')
...
huc2_utm <-        st_transform(huc2_sf, crs = 26917) #geometry col is now in meters!
```
...
neuse_intersect_1 <- counties_sf_utm[neuse_sf,] #rows,columns

#Select intersecting counties using the `filter()` command
neuse_intersect_2 <- counties_sf_utm %>% 
  filter(st_intersects(x = ., y = neuse_sf, sparse = FALSE)) ###Doesn't work for me

#Another way to do the intersect 1
neuse_intersect_3 <- counties_sf_utm %>% st_filter(neuse_sf)

the_filter = st_intersects(x = counties_sf_utm, y = neuse_sf, spares = FALSE)
#View the result
...
tri_sf <- counties_sf_utm %>%
  filter(NAME %in% c('Chatham', 'Durham', 'Orange', 'Wake' ))

  
#Grab the intersecting HUC_8s
#HUC_intersect <- tri_sf %>% st_filter(huc8_sf)
HUC_intersect_sf <- tri_sf %>% 
  st_intersection(huc2_utm)

#Intersect the HUC_8s (aka create new features, basically a clip function in ArcPro)
