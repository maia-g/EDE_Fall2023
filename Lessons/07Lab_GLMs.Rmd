...
```{r}
#Exercise 1: Density plot means y axis is frequency, so you need to specify Ozone as x
oz_density_plot <- 
  ggplot(EPAair, aes(Ozone, fill=factor(Year))) + #this makes r know to make Year into a factor with two levels (2018 and 2019)
  geom_density(alpha = 0.2) + #alpha changes the transparency
  theme_bw() + #this changes the colors, can also use "scale_fill_manual(values = c("darkgreen", "lightblue"))
  labs(x = "Ozone AQI value",
       y = "Density",
       fill = "") #this makes the legend title be gone

oz_density_plot

#Excercise 2:
oz_boxplot <- 
  ggplot(EPAair, aes(x = factor(Year),
                     y = Ozone)) +
  geom_boxplot() +
  stat_summary(geom = "text", #this lets us add stats to our plot easily
               fun = max,
               vjust = -1,
               size = 4,
               label = c("b", "a")) +
  labs(y = "Ozone AQI Value",
       x = "") +
  ylim(0,150)
...
oz_violin <- 
  ...
  stat_summary(geom = "text", #this lets us add stats to our plot easily
               fun = max, #this can be anything, mean, median, quartiles
               vjust = -1,
               size = 4,
               label = c("2018 Max", "2019 Max")) +
  labs(y = "Ozone AQI Value",
       x = "") +
  ylim(0,150)
...
PM2.5_Oz.regression <- 
  lm(EPAair$PM2.5 ~ 
       EPAair$Ozone)
summary(PM2.5_Oz.regression)
#p-value: < 2.2e-16; R-squared: 0.1314
