...
the_registrant <- webpage %>% 
  ...
  #took this from the website after using the google selector tool until only the thing we wanted was green
  html_text() #give me the value associated with the node above
the_registrant
```
...
avg_withdrawals <- webpage %>% 
  ...
  #used selector gadget to get two columns from the table
  html_text()
avg_withdrawals

# Notice they are in a matrix format of characters
...
df_withdrawals <- data.frame("Month" = rep(1:12), #used rep commmand that covers sequence of numbers 1-12
                             "Year" = rep(2000,12), #used rep command to make a vector of 12 instances of the year 2000
                             "Avg_Withdrawals_mgd" = as.numeric(avg_withdrawals), #change character to number
...
df_withdrawals <- df_withdrawals %>% 
  mutate(Registrant = !!the_registrant,
         Facility_name = !!the_facility_name, # !! refers to a variable which we scraped early
...
the_base_url <- 'https://www.ncwater.org/WUDC/app/WWATR/report/view' #before getting into parts of the website
the_facility <- '0004-0001' #found on the site
the_year <- 2019
the_scrape_url <- paste0(the_base_url, '/', the_facility, '/', the_year) #paste0 eliminates adding a space between the called elements
print(the_scrape_url) #builds the website url that we want to pull from
...
ggplot(df_withdrawals,aes(x=Date,y=Avg_Withdrawals_mgd)) + 
  geom_line() + 
  geom_smooth(method="loess",se=FALSE) +
  labs(title = paste(the_year,"Water usage data for",the_registrant), #using 'the_year' so that our plot title changes when we update the variable
       subtitle = the_facility_name,
       y="Withdrawal (mgd)",
       x="Date")
...
#Create our scraping function using the code above, with two inputs: year and facility
scrape.it <- function(the_year, the_facility){
  ...
  #Tells R to wait for 5 seconds (could change this as well); do this when you are bulk scraping so the server doesn't think you are attacking them
...
```{r test.the.function}
#Run the function we created, calling the year and the facility you want
...
the_years = rep(2018:2020) #creating a sequence of years by using rep command to create a vector
my_facility = '0004-0001'

#Use lapply to apply the scrape function
the_dfs <- lapply(X = the_years,
                  FUN = scrape.it,
                  the_facility=my_facility) #this makes the second input (facility) be static

#OR, use purrr's map function (same thing as above, but maps values onto a function)
...
#Teaching computer to look at the links and open them
#Set the web page
...
the_facility_ids <- the_main_website %>% 
  ...
  html_attr("href") %>%           #Pull the "href" attribute from the node, using html_text would return "View Report"
...
#Now you could iterate by replacing the ids in a loop using the map function, see next chunk
```
...
the_facility_ids_subset <-  sample(the_facility_ids,20) #choosing 20 facility IDs atr random
...
dfs_2020 <- map2(the_years, the_facility_ids_subset, scrape.it) #map2 lets you iterate through 2 values, in parallel so first two lists you input need to be the SAME LENGTH 
...
the_facilities <- sample(the_facility_ids,3) #randomly grabbing 3 sites
the_years = c(2018,2019,2020) #if you used map2, it would grab site 1 for 2018, site2 for 2019, and site 3 for 2020
...
the_df <- cross2(the_years,the_facilities) %>% #should be list of 9 values (3 sites, 3 years)
  map(lift(scrape.it)) %>%  #gets all the unique combos
  bind_rows() #combining into dataframe
