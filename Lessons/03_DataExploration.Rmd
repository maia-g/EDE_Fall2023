...
> Answer: It shows all the things that come with that package.
...
str(USGS.flow.data) #shows data data frame and the objects in it
dim(USGS.flow.data) #how many rows and cols you have
length(USGS.flow.data) #how many cols

head(USGS.flow.data) #shows first few rows of data (first 6)
head(USGS.flow.data, 10) #the '10' specifies how many rows to show
tail(USGS.flow.data, 5) # shows botom rows of data table

USGS.flow.data[30000:30005, c(3, 8, 14)] #matrix notation to access certain chunk of data frame
...
> Answer: Depends on what you want to do with your dataset. For example with time series data, there are often many missing observations. If the missing obsv are in the beginning, you can remove them BUT when NA are in the middle of the data you need to be more careful. For time series, we DO NOT remove NAs, instead we replace them wiht a different value.

```{r Removing NAs}
USGS.flow.data.complete <- na.omit(USGS.flow.data) #creating NEW OBJECT to store altered data
dim(USGS.flow.data)
dim(USGS.flow.data.complete) #notice that the COLUMNS did not change, only the ROWS
...
Formatting of dates in R: Upper and Lower-case MATTERS
...
USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%m/%d/%y") #replacing the date-time col we have now to one that has the specific format we want
```
...
create.early.dates <- (function(d) {
       paste0(ifelse(d > 191226,"19","20"),d) #basically attaching at 19 or 20 (depending on the date) to the current values already assigned
       })
#
USGS.flow.data$datetime <- create.early.dates(USGS.flow.data$datetime)

#
USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%Y%m%d") #now converting BACK to into date once correct years are attahced 
