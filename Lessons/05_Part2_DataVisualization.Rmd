...
O3plot1 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_gray() #theme_gray is the default so nothing will change
print(O3plot1)

O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_bw() #changing theme to black and white
print(O3plot2)
...
#Defining a theme to create
mytheme <- theme_classic(base_size = 14) + #starting with built in theme from ggplot
  theme(axis.text = element_text(color = "black"), #changing color of axis tex
...
O3plot5 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  mytheme #option 1 calling theme within the plot
print(O3plot5)  

theme_set(mytheme) #option 2 setting the theme before you even begin
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) + #horizontal line and line type (2 means dashed line)
  geom_hline(yintercept = 100, lty = 2) +
  geom_point(alpha = 0.5, size = 1.5) + #alpha determines transparency of the points
  geom_text(x = as.Date("2020-01-01"), y = 45, label = "good", hjust = 1, fontface = "bold") +   # label is what text you want to add (in this case: GOOD), hjust = 1 means left align
  ...
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), #using LIMITS to plot just a section of the data
    date_breaks = "2 months", date_labels = "%b %y") +
  ylab(expression("O"[3]* " AQI Value")) + #changing y labels to be better
...
#showing a few diff color pallettes
...
library(cowplot)
plot_grid(NvsP2, Nutrientplot, nrow = 2, align = 'h', rel_heights = c(1.25, 1)) #specify the number of rows, and how to align them
