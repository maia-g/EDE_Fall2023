...
A **simple feature** is one geographic element along with its attributes. In other words, it's a row in a dataframe when that dataframe includes a column for geometry. 
...
class(EPAair.sf)
names(EPAair.sf)
str(EPAair.sf$geometry)

#View using sf plot
plot(EPAair.sf['count_all'],pch=16)

#View using ggplot
ggplot(data=EPAair.sf) + geom_sf()
...
mapview(sfg_GraingerHall) #Just shows a point on a gray background
...
mapview(sfc_GraingerHall) #Shows a point on a map
