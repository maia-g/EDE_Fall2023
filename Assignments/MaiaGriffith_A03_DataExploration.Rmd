...
###ISSUE: using the './Data/' then tab to see the options yields nothing, but it was working when I had first attempted to do the similar thing in the lesson with the USGS dataset. It doesn't work on the lesson either for me today and I do not know why.
...
###QUESTION: Do I still need to call these out since it is enormous?
#Neonics 
#Litter
...
> Answer: 
 ...
 3. Trap placement is randomized in places with 50% > cover and targeted in areas with less vegetation.
...
```{r Q9}
###QUESTION: Do I need to somehow call out each plot so it shows when I knit or does it automatically show up?
ggplot(Neonics) +
...
```{r Q13}
#Use the length AND unique functions together to return the number of plots sampled.
...
###QUESTION: What are Levels? Google was not helpful...
```

> Answer: Summary returns the plot IDs AND how many times each was sampled. Unique returns all the different plot IDs and excludes repeats. It also includes "Levels" which shows the plot IDs in ascending order.
...
#the Functional Group "Mixed" has the least by quite a lot, but the others are fairly equally distributed.
```
...
#Violin plot does not look very good but I think I did it right.
...
> Answer: The boxplot is more effective in this case because there are not a huge amount of data points to show any visible distribution. When we used the USGS dataset in the lesson, there were thousands of data points so the width of the violin plots were actaully discernable.
