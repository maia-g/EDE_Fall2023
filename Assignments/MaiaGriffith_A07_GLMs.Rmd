...
#5 Scatter plot of temp by depth (x = depth, y = temp) with line showing linear model, temp from 0 to 35
temp_depth_plot <-
...
#10 New regression using depth AND daynum to explain temperature.
tempdepthdaynum.regr <- lm(data = Jul.lake.chem.phys, 
                           temperature_C ~ depth + daynum)
summary(tempdepthdaynum.regr)

```

11. What is the final set of explanatory variables that the AIC method suggests we use to predict temperature in our multiple regression? How much of the observed variance does this model explain? Is this an improvement over the model using only depth as the explanatory variable?

> Answer: The AIC method suggests we use depth (high impact) and daynum (some impact) in our multiple regression. This new model explains 74.1% of the variability in temperature, which is a small improvement over only using depth.
...
#12
# Wrangle the data
Jul.Temp.Avg <- Jul.lake.chem.phys %>%
  group_by(lakename, daynum, depth) %>%
  summarise(temperature_C = mean(temperature_C))

#summary(Jul.Temp.Avg) # Use this and following to check data
#summary(Jul.Temp.Avg$lakename)

# Format ANOVA as aov
Jul.Temp.Avg.anova <- aov(data = Jul.Temp.Avg, temperature_C ~ lakename)
summary(Jul.Temp.Avg.anova)

# Format ANOVA as linear model (lm)
Jul.Temp.Avg.anova.lm <- lm(data = Jul.Temp.Avg, temperature_C ~ lakename)
summary(Jul.Temp.Avg.anova.lm)
...
temp_depth_ANOVA_plot <-
  ggplot(Jul.Temp.Avg, aes(x = depth, 
                           y = temperature_C,
                           color = lakename)) +
  ...
  labs(y = "Temperature (C)",
       x = "Depth (meters)",
       color = "Lake Name",
       title = "Average Temperature by Depth in Each Lake") +
  ylim(0,35)
...
```{r tukey.test}
#15
TukeyHSD(Jul.Temp.Avg.anova)

```

16.From the findings above, which lakes have the same mean temperature, statistically speaking, as Peter Lake? Does any lake have a mean temperature that is statistically distinct from all the other lakes?

>Answer: Central Long Lake, Crampton Lake, East Long Lake, Tuesday Lake all have the same mean temperature, statistically spekaing, as Peter Lake. It does not look like any lake is statistically distinct from all other lakes.
...
>Answer: The results give a p-vale > 0.05, therefore we fail to reject the null hypothesis and can conclude that there is not a signficant difference in the mean temperatures between the two lakes. This does match my number 16, because the TukeyHSD test resulted in a p-adj > 0.05 for Crampton and Ward lakes, which also meant that there was not a significant difference between the temperatures of those lakes. 
