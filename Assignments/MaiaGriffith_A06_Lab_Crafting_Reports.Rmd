...
#Import libraries
library(tidyverse);library(lubridate);library(here);library(knitr);library(tinytex)
...
```{r Ozone 2018, echo=FALSE, fig.height = 3, fig.width = 4, fig.align='left', fig.cap= 'Ozone Levels by Month (2018)'}
Oz18_plot <-
  epa_data %>%
  filter(Year == 2018) %>% #Only want 2018 records
  ggplot(aes(x = factor(Month,
                        levels = 1:12, #make sure 12 box plots
                        labels = month.abb),
             y = Ozone)) +
  geom_boxplot() +
  labs(title = "Ozone Levels by Month (2018)", ####Plot title, maybe don't need this with the fig.cap
       y =expression(paste("Ozone ","(", mu, g/m^{3}, ")")), #Making the symbols
       x = "Month") +
...
```{r Ozone 2019, echo=FALSE, fig.height = 3, fig.width = 4, fig.align= 'left', fig.cap= 'Ozone Levels by Month (2018)'}
Oz19_plot <-
  epa_data %>%
  filter(Year == 2019) %>% #Only want 2019 records
  ggplot(aes(x = factor(Month,
                        levels = 1:12, #make sure 12 box plots
                        labels = month.abb),
             y = Ozone)) +
  geom_boxplot() +
  labs(title = "Ozone Levels by Month (2019)", #Plot title
       y =expression(paste("Ozone ","(", mu, g/m^{3}, ")")), #Making the symbols
       x = "Month") +
  scale_x_discrete(name = "Month", drop = FALSE) +
  theme(axis.text.x = element_text(angle = 45,  hjust = 1),
